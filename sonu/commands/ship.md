...
- **Adopt it or create it in Phase 0** — never blindly create — and **rewrite it at the end of every phase** with the current facts, one per line: `repo:`, `base:`, `branch:`, `pr:`, `mode:`, `disposition:`, `security_surface:` (`met` / `not-met` — the security-surface test evaluated once in Phase 1.5; Phase 2B and Phase 6 read it instead of re-judging), `phase_done:`, `prepr_passes:`, `prepr_reviewed_sha:`, `cycles_used:`, `prev_at:`, `handled_comment_ids:` (comma-separated), `reviews_skipped:` (comma-separated; empty is a real value meaning "nothing skipped"), `delegated_fixes:` (count, incremented at each delegation — the final report reads it), `open_items:` (anything mid-flight), and — stacked runs only (see Stacked PRs) — `own_commits:` (this PR's own commit SHAs, oldest first; recorded because after a parent squash-merges it can no longer be recomputed).
...
**Why a typed `full` is the one exception.** The mode scales review *depth*, not *surface* — a diff with no security surface has nothing for `/security-review` to find at any effort level, which is why `auto` obeys the test even when it promotes itself to full. But the test is a hand-written list with no test suite behind it, and a human who types `full` is rare, deliberate, and usually suspicious about something the heuristic cannot see. That typed override is the escape valve for a heuristic being wrong; do not extend it to `auto`'s self-promotion, which is a line count, not a judgment.
...
6. **Ledger after every pass:** update `prepr_passes:` and `prepr_reviewed_sha:` (the HEAD SHA the last completed review actually covered). **After every pass, also write `security_surface:`** — on a fan-out, read it straight off self-review's domain-lens line (its step 5 names, for each domain lens, the clause that matched or that none did): security dispatched → `met`, security skipped → `not-met`. On the inline pass, which emits no such line, judge that skill's security condition against the diff yourself.

Two rules keep that verdict honest, and both exist because a cached verdict nobody re-checks is how a late fix ships unreviewed:

- **Re-evaluate on every pass here and every cycle in Phase 6**, against the lines that pass or cycle *added* — new files and new lines in files already read alike. A fix commit can introduce a security surface the first pass never saw.
- **The verdict is monotonic: once `met`, it stays `met` for the rest of the run.** Only `not-met` is ever revised, and only upward. Re-evaluation can add review, never remove it. On resume after a compaction or interruption, those two fields say exactly which pass you're in and what the next delta diff is — re-derive from the ledger, not from memory.
...
Invoke `/security-review` on the diff — or **skip it when the ledger's `security_surface:` reads `not-met` and the mode is not a `full` the user typed**, recording the skip in `reviews_skipped:`. Capture findings in the same shape. Treat them like code-review findings (no external comment, no thread).
...
2. Wait for activity **newer than `$PREV_AT`** using the explicit loop below. Run it as a background until-loop (do NOT foreground-sleep).
   ...
   **CRITICAL: run this loop separately from the Phase 7 CI poll. Never combine them into one loop.** Two conditions (re-review timestamp AND CI buckets) cannot be safely merged — the variable-capture patterns are incompatible and a combined loop will stall. Run this re-review loop first, collect new findings, then run the CI poll loop in Phase 7.
   ...
   ```
   Re-run `/security-review` on the new diff on the same terms as Phase 2B — the ledger's `security_surface:` verdict, or a typed `full`. **Re-evaluate that verdict against the lines this cycle added** — new files and new lines in already-seen files alike, never file-novelty alone — and remember it only ever escalates to `met`.
