...
| **auto** | evaluate these in order, **first match wins**: security-surface test met → treat as **full**; else trivial (the `light` row's definition) → treat as **light**; else > 200 changed **code** lines → treat as **full**; else medium effort | run only when the security-surface test (below) passes — **including when this row promotes itself to full** | up to 3 cycles |
| **`full (typed)`** — a human typed a `full` synonym; this exact token is what the ledger records and what Phase 1.5 sub-step 1c matches | high effort | **always** | up to 3 cycles |
...
Generated churn — fixtures, snapshots, lockfile noise beyond what the security examples already catch — counts as code and may promote a mechanically safe diff to full. That over-spend is deliberate and must not be "fixed": deciding *was this generated?* is exactly the judgment that a Sonnet-class and a Fable-class reader would answer differently, and a predictable mode is worth more than the saved effort tier. A human who knows better types `light`.

**A promoted `full` is not `full (typed)`.** When the `auto` row promotes itself, record plain `mode: full` — the review *depth* becomes full, and that row's security-surface rule still governs. Only a human typing a `full` synonym ever writes `full (typed)`, and only that exact token unlocks the always-run override; a plain `full` in the ledger never does.
...
1. **Pass 1 — review what has not been reviewed yet.** `Skill(sonu:self-review)`, scoped by the ledger's `prepr_reviewed_sha:`:
   - **Same session, same bytes — adopt build's review instead of re-running it.** If this session ran `/sonu:build`, and its built summary's diff stat and untracked-file list are byte-identical to what Phase 0 step 3 printed, and the branch is exactly one commit ahead of the base, then build's Phase 3 risk list **is** pass 1's self-review output — do not run the skill again on the same content. Sub-steps 1b and 1c below still run. Any difference in those outputs, a branch more than one commit ahead, or an uncertain memory of what build printed → re-run the review; this shortcut only ever skips a review you can prove already happened. The commit-count check (self-contained — copy it exactly):
     ```bash
   ...
   Then, **after step 6 has written `security_surface:` for this pass**, run the two Claude reviews. Order inside a pass is fixed: self-review → write the verdict → 1b → 1c → partition (step 2). Their findings join the self-review list and are partitioned with it.

   - **1b. Claude code review — pass 1 only.** Per the effort mode, invoke `/code-review` (low/medium/high) — or skip in `light` on a trivial diff, recording the skip in `reviews_skipped:`. Capture findings as `{file, line, description, severity}`. Passes 2+ do not re-run it: self-review's delta pass covers the fix commits, and re-running a whole-branch review on every pass is the spend this loop exists to avoid.
   - **1c. Claude security review — once pre-PR.** Invoke `/security-review` on the whole branch diff (`git diff origin/<base>...HEAD`) in the first pass whose `security_surface:` reads `met`, or in pass 1 whenever `mode:` is `full (typed)`. **Skip it when the verdict reads `not-met` and the mode is not `full (typed)`**, recording the skip in `reviews_skipped:`. Because the verdict is monotonic (step 6), a pass that first flips it to `met` runs this sub-step then — a later pass never does; Phase 6 re-runs it per cycle on its own terms. Capture findings in the same shape as 1b; no external comment, no thread.
...
4. **Re-review the delta.** Run `Skill(sonu:self-review)` again scoped to what changed since the last reviewed state: `git diff <prepr_reviewed_sha>..HEAD` plus the full content of any file the fixes touched. Then re-evaluate `security_surface:` (step 6) and run sub-step 1c if the verdict first reads `met` on this pass. New findings → back to step 2 with only those.
...
Merge all sources — AI bot findings and **human inline comments** from Phase 2 (your own Claude reviews were already fixed or justified in Phase 1.5; their `JUSTIFY` reasoning is reused verbatim when a bot raises the same point) — into one deduplicated list:

- **Duplicate** (any two sources flag the same file+issue): one entry, address once. (Bots overlap a lot — expect heavy dedup.)
- **Valid and consequential** → `FIX`: the finding names a concrete failure (an input or sequence that produces wrong behavior), a security or data-loss exposure, or a convention the repo states (`CODING.md` / `CONTRIBUTING.md`).
- **Valid but harmless** → `JUSTIFY`, with the "Keeping as-is" wording: hardening for input that cannot arrive — *only* when you can name the validated boundary upstream that already rejects it, in the reply; an unnamed "can't happen" is a `FIX` — extractions, renames, docstring or comment requests, "consider" suggestions. A finding that names a security or data-loss exposure is never in this bucket, whatever severity label the reviewer gave it. A `FIX` commit is fresh material for every bot's next pass, so a fix that removes no failure buys a re-review round and returns nothing.
...
**From cycle 2 on, the `FIX` bar rises.** A new finding is `FIX` only if it sits on a line this cycle's fix commits changed, or clears Phase 3's `FIX` bar on its own — a concrete failure, a security or data-loss exposure, or a stated repo convention (all three arms, not just the first). Every other finding — including a first-time nit on code no cycle touched — is `JUSTIFY`. Reviewers are non-deterministic and re-roll unchanged code on every pass; treating each re-roll as new work is the treadmill the cycle cap exists to stop.
...
- **Re-review cycles used** — read from `cycles_used:`. When it is 2 or more, add one line: *"Two or more bot rounds — consider configuring the reviewers to review once per PR and report Important-only after the first pass; the per-bot settings are in pr-conventions' `references/reviewer-tuning.md`."*
...
- **Per-bot review-once and severity settings** (the final report's cycle hint) — not stored here; they live, dated, in pr-conventions' `references/reviewer-tuning.md`.
