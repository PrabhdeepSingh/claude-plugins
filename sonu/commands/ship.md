...
**Read the reviewer's own severity label first.** Every reviewer here labels its findings, and the label is the comment's first line; `Skill(sonu:pr-conventions)` Section D is the one home for how to read it. The label sets the default the bullets above then refine: Critical and Major are `FIX` whatever the effort; Minor is `FIX` only when it names a concrete failing input, and `JUSTIFY` otherwise; an unlabeled finding is triaged on the bullets alone; anything naming security, data loss, or a stated repo convention is `FIX` at any label. Do not hunt for `Nit:`/`Optional:`/`Consider:` prefixes — on 129 audited threads that vocabulary never appeared once, and an executor looking for it treats everything as required.
...
**One push per cycle.** Apply every `FIX` from this cycle's triage, run the suite once, commit (several commits are fine when revert-ability wants them), and push **once**. A second push inside a cycle is a new cycle: the count that governs the cap is pushes after the PR opened, read from the PR itself (the ledger contract's fence), so a mid-cycle push is not forbidden by prose, it is counted. Never push a docs-only or comment-only change on its own — fold it into the cycle's commit. In one audited week, 44 pushes after open bought 72 bot reviews, roughly 200 CI runs, and five exhausted CodeRabbit allowances.
