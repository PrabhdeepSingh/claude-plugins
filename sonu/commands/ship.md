...
1. **Capture each bot's current latest review timestamp first** — a rerun must wait for activity *newer* than what's already there, or it exits instantly on the existing reviews. Echo the value; step 2's loop needs it as a literal:
   ...
   - **Every other bot that participated in Phase 2:** drop its re-review mention as an issue comment — `@coderabbitai review`, `@sourcery-ai review`, `@greptileai`, `@ellipsis-dev`, `@cubic-dev-ai`, `/review` for Qodo (Aikido and Korbit have no mention; they re-run on push or not at all). Do this for **all** of them, not just the ones that "don't auto-review": a repo configured to review once per PR (pr-conventions' `references/reviewer-tuning.md`) re-reviews *only* on this mention, and you cannot tell from the PR which repos those are. A bot that also re-reviews on push gets one redundant run — cheaper than a bot that silently never re-reads the fix.
