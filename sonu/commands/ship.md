...
|------|---------------------|--------------------------|----------------|
| **light** | low effort; skip entirely if the diff is **trivial** — ≤ ~10 changed **code** lines by the code-line count below, and only CSS/markup/docs/config/comments, no logic | run only when the security-surface test (below) passes | exactly 1 |
...
Invoke `/security-review` on the diff — or **skip it when the ledger's `security_surface:` reads `not-met` and its `mode:` is not `full (typed)`**, recording the skip in `reviews_skipped:`. Capture findings in the same shape. Treat them like code-review findings (no external comment, no thread).
