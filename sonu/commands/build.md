...
Everything else in this phase still runs, in-chat instead of in plan mode: **skip steps 1 and 5** (`EnterPlanMode` and `ExitPlanMode`) and run steps 2, 3, and 4 exactly as written, printing the tree in-chat instead of into a plan file. When a fork needs a genuine product decision the spec does not answer, stop and put the precise blocker on the ticket rather than asking in chat or guessing — the ticket is where that decision belongs. Phases 0, 2, and 3 are identical in both doors.

For the ad-hoc door (everything not routed from a ticket), enter plan mode to run the design phase. That is the only pause in the flow — your approval of the design is the gate.

1. `EnterPlanMode` — switches to read-only; the design tree will be written into the plan file. *(Ticket-driven: skipped.)*
