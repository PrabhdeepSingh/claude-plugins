...
2. **Load the quality bars as design constraints** — `Skill(sonu:code-standards)` always, plus **only** the bars the Phase 0 surface flagged, as `Skill(sonu:<name>)`, plus `Skill(sonu:performance)` when Phase 0's kind is optimization; a bar whose surface wasn't flagged — and whose kind isn't optimization — is not loaded. These are the same bars Phase 2 builds under; loading them before the tree is drawn keeps the approved design from conflicting with them. (Loading a skill is read-only — legal in plan mode.)
