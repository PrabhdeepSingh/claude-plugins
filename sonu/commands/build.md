...
**Continuing a multi-PR plan?** Before triage, on either door, check for a build handoff at the ship ledger's two-home path (Phase 3 step 4 names it). A handoff whose `Branch:` is not this branch, or whose `Slices:` line has no `next` or `pending` entry, is stale — delete it and continue as a fresh build. A handoff that matches and still has undone slices is adopted: its `Plan:` is this run's plan, and Phase 1 takes the `next` slice.
