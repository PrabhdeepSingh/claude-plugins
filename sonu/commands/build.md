...
4. `Skill(sonu:model-tiering)` — after the plan file's implementation steps are written (step 3 leaves them meeting the executor-ready bar), grade the delegable ones per that skill when a trustworthy executor tier sits below the session model; otherwise it no-ops (the skill self-detects). If the plan has no implementation steps yet, finish writing them first — there is nothing to grade.
