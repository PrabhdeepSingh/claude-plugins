...
1. `EnterPlanMode` — switches to read-only; the design tree will be written into the plan file. *(Ticket-driven: skip — do not call this.)* **Continuing a multi-PR plan?** Before drafting anything, check for a handoff file at the ledger's two-home path (Phase 3 step 4 names it). If it exists and its `Plan:` line points at a plan file with slices still undone, adopt that plan: read it, take the next undone slice as this run's task, and skip the interview — the answers are already recorded under its Design Tree. Draft a new plan only when no handoff names one.
...
3. Print a brief **built summary**. **Green means green as of the last edit:** the summary may call the gates green only if the ladder in Phase 2 step 4 ran *after* the final change to the working tree — including any fix step 1's self-review produced — and it names the run that backs the claim ("gates re-run after the null-check fix"). If anything was edited since the last run, re-run before writing; quoting an earlier run is claiming a result nobody has seen.
   ...
   - **What was deliberately not touched**: adjacent problems noticed but out of scope (an unused import in a file you only read, a similar gap in a sibling module), each with "separate task?" — this line proves scope discipline rather than asserting it, and hands the owner the follow-ups instead of silently absorbing or silently dropping them. Plan and handoff documents belong here too, as their own follow-up commit rather than bundled with the code — on the audited PRs, Markdown files drew more review threads than security did.
