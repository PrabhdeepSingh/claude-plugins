...
**Continuing a multi-PR plan?** Before triage, on either door, check for a build handoff at the ship ledger's two-home path (Phase 3 step 4 names it). A handoff whose `Branch:` is not this branch, whose `Head:` is not an ancestor of the current HEAD (`git merge-base --is-ancestor <Head> HEAD`), whose `Plan:` names a file path that does not exist (a ticket reference in `Plan:` is validated by the ticket route instead), or whose `Slices:` line has no `next` or `pending` entry, is stale — delete it and continue as a fresh build. A handoff that matches and still has undone slices is adopted: its `Plan:` is this run's plan, and Phase 1 takes the `next` slice.
