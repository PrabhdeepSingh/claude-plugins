...
- **Age and adoption are checkable, so check them before adopting.** A package almost nobody depends on, published in the last few months, maintained by one person, is not disqualified — but it is a decision to make deliberately rather than by autocomplete: state the three numbers (roughly how many projects depend on it, how old the first release is, how many maintainers) and why the dependency is still worth it. Never take a release published in the last few days when an older one will do — a compromised or hijacked publish is caught within days, and a three-day wait costs nothing. And read the name character by character against the one you meant: a substitution (`djang0` for `django`), an added or dropped hyphen, a Cyrillic homoglyph (`а` for `a`) — that is the whole typosquat attack, and it survives every audit tool because the malicious package has no advisories.
