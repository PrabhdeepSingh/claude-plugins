...
- **Reader model — set explicitly on the Agent call, every time.** The `model` argument is the cheapest trustworthy executor tier below this session on [[model-tiering]]'s ladder (the exact value and the why live in `references/lenses.md`'s dispatch mechanics); an Agent call with no `model` runs the reader at the session's own tier and price. Dispatching a read-only reader is not the delegation that skill's Section 4 forbids — the reader gathers evidence; every accept/reject decision stays in this session.
