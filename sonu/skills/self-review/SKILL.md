...
- **Each lens reports findings as `Risk (<tag>): <what> — <why> [file:line]` lines with a confidence tag** — the parenthesized tag is the lens name, or `CODE/<checklist>` for the code lens — and must reply "Nothing in my lens." rather than invent a finding to seem useful.
- **Each lens reads on a budget and reports under a cap** — both live in the shared frame: bounded reads of the hunk's enclosing block only, no whole files, no repo search outside the consumers lens; at most five findings, high or medium confidence only, then one `Withheld: N more.` line. A lens that reads whole files or emits a page of findings is spending the tokens this gate exists to save — and the withheld line is a signal to look at that lens's domain yourself, never a reason to re-dispatch.
