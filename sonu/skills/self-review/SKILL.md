...
- **The code lens — one subagent carrying the correctness, test-adequacy, and silent-behavior-change checklists — dispatches whenever the diff changes executable code**, meaning changed lines in non-doc files. Any code diff can carry a logic error, an untested path, or a changed default, so this lens has no narrower precondition than "there is code here." One reader with three checklists costs one read of the diff; three readers cost three, and the only thing they added — three readers co-flagging one code issue — is a signal the domain lenses still supply across lens boundaries, while step 4's default-reject bar carries the rest. One carve-out, because it decides the common case: a non-doc file changed **only** in metadata — a version string, a lockfile hash, a copyright year — is not code for this purpose. Nearly every release rides a version bump alongside its real change, and counting those two lines as code would make the prose path below unreachable in exactly the repos that need it. **Read this off the diff, not off step 2's number:** in a docs-product repo step 2 deliberately counts *all* changed lines including prose, and that count answers "is this diff big enough to fan out," never "is there code here." Confusing the two dispatches the code lens against a pure prose diff, which is exactly the waste this gate exists to stop.
...
The code lens goes out only where the prose carries at least one of its checklists' domains — **correctness** when it states rules, thresholds, or branches an executor follows; **silent-behavior-change** when it changes what a component does without announcing it; **test-adequacy** only when the repo has a suite whose adequacy this prose changes (usually it does not) — and its prompt keeps only the checklist paragraphs whose domain is present, dropping the rest.

If nothing above fires — a typo pass, a wording cleanup, a README polish — say the diff is low-risk and fall back to the inline pass (3a) rather than dispatching anything at all. Five agents on a doc edit is the failure this gate exists to prevent.
...
- **Each lens reports findings as `Risk (<tag>): <what> — <why> [file:line]` lines with a confidence tag** — the tag is one of exactly seven: `SECURITY`, `DATA INTEGRITY`, `CONSUMERS`, `INTERFACE`, `CODE/correctness`, `CODE/tests`, `CODE/silent-change` — and must reply "Nothing in my lens." rather than invent a finding to seem useful.
- **Each lens reads on a budget and reports under a cap** — both live in the shared frame: bounded reads of a flagged hunk's enclosing block or its one-hop callee, no whole tracked files (untracked files *are* the diff and are read in full), and repo search only in the two places the frame allows — the consumers lens hunting consumers, the code lens's tests checklist looking up a changed function's tests — each capped; at most five findings, high or medium confidence only, then one `Withheld: N more.` line. A lens that reads whole files or emits a page of findings is spending the tokens this gate exists to save — and the withheld line is a signal to look at that lens's domain yourself, never a reason to re-dispatch.
...
- Did the output name, for every gated lens — the four domain lenses always, the code lens too when the prose path gated it — the clause that matched or that none did, so both an over-firing and an under-firing gate are visible?
- Did every lens reply stay within the five-finding cap — and did a `Withheld:` line make you read that lens's domain yourself rather than re-dispatch?
...
|---|---|
| `references/lenses.md` | The dispatch prompt templates for both tiers — the code lens with its three checklists and the four domain lenses, plus the shared frame's read budget and output cap — read when dispatching the step-3b fan-out. The conditions themselves live in step 3b, not here. |
