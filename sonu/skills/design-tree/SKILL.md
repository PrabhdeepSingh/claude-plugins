...
- **Prune before the gate.** Read the finished plan once more as the reviewer who will reject padding, and put four tests to every item: does it change what an executor *does* or what the owner must decide at the gate (a note that alters neither is cut — a `[?]` fork alters the second, so it stays); does a cheaper home exist (a sentence in an existing section beats a new section, lens, or file); does its recurring cost outlive the incident that motivated it (an extra subagent per run, a count kept in sync across files); is it flexibility no stated requirement needs (an interface with one implementation, a config nobody sets, a layer with one caller)? Present the trims and the kept-anyway items, each with its reason, in the message that carries the plan — the owner rules on trims at the gate, not after the build.
...
- Did you run the prune pass, and does the message presenting the plan list the trims and the kept-anyway items with their reasons?
