...
- **Prune before the gate.** Read the finished plan once more as the reviewer who will reject padding, and put four tests to every item: does it change what an executor *does* or what the owner must decide at the gate (a note that alters neither is cut — a `[?]` fork alters the second, so it stays); does a cheaper home exist (a sentence in an existing section beats a new section, lens, or file); does its recurring cost outlive the incident that motivated it (an extra subagent per run, a count kept in sync across files); is it flexibility no stated requirement needs (an interface with one implementation, a config nobody sets, a layer with one caller)? Record the trims and the kept-anyway items, each with its reason, in the plan itself — a short `Pruned:` list directly under the Design Tree — so the record the owner approves carries them; where no plan file exists (an in-chat tree, a ticket-driven build) the same list goes in the plan-settled note. The owner rules on trims at the gate, not after the build.
