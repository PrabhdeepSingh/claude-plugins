...
name: accessibility
description: >-
  Accessibility engineering for product interfaces — keyboard support, focus states, ARIA, forms, screen readers, hit areas, motion and zoom. INVOKE PROACTIVELY when building or reviewing any UI component, modal, menu, form, or custom widget, and on any keyboard or screen-reader complaint — even when nobody says "a11y". Contrast measurement: [[colors]]; text sizing and iOS input zoom: [[typography]]; spatial RTL: [[layout]]; whole-screen audits: [[interface-review]].
---
...
Rendered-pair contrast measurement and color remediation are covered by [[colors]]; visual text sizing and iOS input zoom by [[typography]]; spatial RTL layout by [[layout]].
...
→ `references/semantics-and-aria.md` — native-element choices, button vs. link, landmarks, accessible-name computation, and disabled-state handling, read when this change adds a custom widget, an ARIA attribute, or a page-structure element.
...
→ `references/focus-and-keyboard.md` — focus-ring recipes, skip links, tabindex rules, focus trapping, and the per-widget APG keyboard patterns, read when this change adds a focusable control, an overlay, or a composite widget.
...
→ `references/hit-areas.md` — target-size exceptions, the pseudo-element expansion pattern, and the collision rules, read when this change adds or resizes a small or icon-only control.
...
→ `references/forms.md` — label patterns, the `autocomplete` and input-type tables, and worked error-messaging markup, read when this change touches a form, an input, or validation.
...
→ `references/motion-and-zoom.md` — the reduced-motion query patterns, autoplay and timed-UI rules, and the 200% zoom / 320px reflow and rem-vs-px guidance behind §14, read when this change adds motion, autoplay, or timed UI, or when checking zoom and reflow behavior.
...
→ `references/screen-readers.md` — visually-hidden techniques, live-region and toast patterns, and alt-text and SVG handling, read when this change announces dynamic content or adds an image, icon, or inline SVG.
...
|---|---|
| `references/focus-and-keyboard.md` | Focus rings, skip links, tabindex, focus trapping, and the APG keyboard patterns (§2, §3, §4) |
| `references/semantics-and-aria.md` | Native elements first, button vs. link, landmarks, accessible names, disabled states (§1, §8, §13) |
| `references/forms.md` | Labels, autocomplete, error messaging, input types (§6, §7) |
| `references/screen-readers.md` | Visually hidden content, live regions, toasts, alt text, SVG (§11, §12) |
| `references/hit-areas.md` | Target sizes, expanding hit areas, collision rules (§5) |
| `references/motion-and-zoom.md` | `prefers-reduced-motion`, autoplay and timed UI, 200% zoom, reflow, rem vs px (§10, §14) |

## Provenance and maintenance

Last verified 2026-07. The volatile claims here are platform-behavior claims, not methodology: `inert` and `overscroll-behavior` support, forced-colors-mode behavior, screen-reader handling of dynamically inserted live regions, and the WCAG success-criterion numbers and thresholds cited in §5, §13, and §14. Re-verify against current browser-support tables and the current WCAG specification before treating any specific number or support claim as still true; the principles themselves hold regardless.
