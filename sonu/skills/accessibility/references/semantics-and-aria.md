...
- Don't pre-disable submit buttons for invalid input: keep them enabled, validate on submit, and focus the first error. Once a submission is genuinely in flight, disabling with a spinner is the correct state (see `references/forms.md`).
