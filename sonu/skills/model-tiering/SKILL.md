...
These four are the *whole* test. A step that clears all four **should** carry a tag — that is the skill working, not optional caution to be overridden by a hunch. Do not withhold a tag on grounds outside the four ("it's sensitive," "pattern-heavy," "feels risky"): if a worry doesn't reduce to a specific failure of criterion 1, 2, 3, or 4, it is not a disqualifier. In particular, the effort you spent making a step executor-ready is what *makes* it delegable — once its paths, conventions, and check are settled in place, "the patterns are sensitive" no longer holds, because the patterns are now written down (criterion 2). A step is kept up because it fails a criterion, never because specifying it well made it feel important.
