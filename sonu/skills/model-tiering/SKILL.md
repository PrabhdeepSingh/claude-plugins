...
**Why the ladder has a floor, and why it is not price.** Cost scales with *turns*, not with per-token price: a model one rung too cheap routinely takes two to three times the turns on anything multi-step, and pays for the saving twice over in wall-clock and in the orchestrator's context. That is why the bottom of the ladder sits below this skill's bar rather than being the default: the cheapest tier is right only where the step's text *is* the work — transcription of decisions already written — and a mid tier is the floor for anything the executor must reason from prose. The same arithmetic points the other way at the session: an orchestrator run at a lower effort does not get cheaper, it gets longer, because reasoning is what buys turn efficiency. Cheapen the mechanics; never cheapen the driver.
...
For each tagged step, spawn a subagent (the Agent tool in Claude Code), model set per Section 2's mapping against the Provenance ladder — **and pass that tier's `model` value explicitly on every Agent call.** Omitted, the harness runs the subagent on the session's own model: the subagent then costs exactly what the session costs and this skill has done nothing. An Agent call without `model` is a bug, not a default. The prompt is the step's text verbatim plus the exact paths and settled conventions it names — nothing more, and files referenced by path, never pasted, since the executor can open what it needs and a pasted copy costs both sessions. **Every delegated prompt carries the halt list**, as a closed set: stop and report if the step's check still fails after three attempts; if the step needs a dependency, credential, or config it does not name; if the text turns out ambiguous once the named files are open; or if a test that was passing before the step starts failing. On any of them, report what was attempted, what the check printed, and what is missing — and change nothing further. Three is a ceiling, not a quota: a check that fails twice for the same reason is already a specification problem, and the orchestrator that wrote the specification is the one equipped to fix it. Without the list the failure is not a stop but a drift — the executor widens its scope until something turns green, and the diff that comes back is the expensive kind to review. **Bound what comes back, too:** say in the prompt that the reply is roughly 1,000–2,000 tokens — what changed, at which paths, what the verification printed, anything it could not do — with no narration of the steps, no pasted file contents, no tool transcripts; a large output is named by path plus the ten relevant lines, and you read the rest only if the check you run yourself fails. When it returns, **run the step's verification yourself**; never accept the subagent's own report. Delegation changes who types, not the bars: any discipline in force for the build — a failing test observed before the implementation, a standards constraint — still applies, and observing it stays in the session.

**Scheduling:** read the plan's `needs:` / `parallel with:` / `touches:` annotations ([[design-tree]] §8 defines them; an unannotated step is sequential) — adjacent tagged steps with no dependency between them and disjoint touched sets may run in parallel — independent slices, tests for already-implemented behavior, docs. Migrations, shared-state changes, and dependency chains stay sequential. Steps that share an API contract need the contract defined first; then they parallelize against it.
