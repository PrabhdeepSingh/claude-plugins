...
| Linked PR open, heartbeat quiet past the ship-idle threshold, no blocker comment | Ship stalled — the pass ran out of turn. Resumable by the next dispatcher pass on the original authorization; PR activity does not count as life here, because bots commenting on an abandoned PR is the symptom, not the pulse. |
