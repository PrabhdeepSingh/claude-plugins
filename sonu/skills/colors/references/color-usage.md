...
How to deploy color in an interface: semantic tokens, meaning, emphasis, and appearance variants. For picking the values themselves, see `references/palette-generation.md`; for checking pairs, see `references/accessibility-contrast.md`.
...
If a role has no token yet, add the token; don't borrow one that happens to have the right value today. In Tailwind projects, this is the `@theme` block; see `references/gamut-and-tailwind.md`.
...
Every custom color needs a light and a dark variant; dark mode derivation is covered in `references/palette-generation.md`. Beyond that, users who enable increased contrast expect visibly stronger differentiation; supply it with `prefers-contrast`:
