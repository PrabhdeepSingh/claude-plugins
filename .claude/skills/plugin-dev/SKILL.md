...
| **`.sonu/` in a managed repo is the queue's own state directory** | `/sonu:factory` reads `.sonu/factory-config.md` (tracker choice, committed so a team shares it) and the local tracker keeps tickets in `.sonu/tickets/`. That directory belongs to the repo being *managed*, never to this one — the plugin ships no `.sonu/`. Credentials never live there; adapters read them from the environment, because config files and ticket text both get committed. Global fallbacks (`~/.sonu/factory-config.md`, `~/.sonu/tracker-adapter.md`) sit beside the memory store's `~/.sonu/memory/`, which set that precedent. |
